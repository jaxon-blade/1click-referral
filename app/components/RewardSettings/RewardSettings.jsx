...
const RewardSettings = ({ title, subtitle, advocateReward }) => {
  const [activeButtonIndex, setActiveButtonIndex] = useState(
    advocateReward?.discountValueType === "fixed" ? 1 : 0,
  );
  const [value, setValue] = useState(
    advocateReward?.appliesTo || "collections",
  );
  ...
  const handleSave = (values) => {
    ...
    shopifyApp.saveBar.hide("reward-advocate");
    shopifyApp.toast.show("Advocaete reward settings saved");
  };
  return (
      ...
      >
        {({ errors, touched, setFieldValue, values, dirty, resetForm }) => (
          <Form>
            ...
            <SaveBar id="reward-advocate" open={dirty}>
